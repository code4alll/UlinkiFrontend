...
const ProductCarousel = ({ products = [] }) => {
  ...
  const convertPrice = (price, fromCurrency) => {
    ...
    if (!rate || !exchangeRates[fromCurrency]) return price;
  ...
  const cartHandler = (id) => {
    ...
    toast(
      <div className="toaster">
        <VerifiedIcon /> {`${value} items added to cart successfully!`}
      </div>,
      {
        duration: 3000,
        position: 'top-center',
        style: { padding: '3px', color: 'rgb(0, 189, 0)' },
        className: 'success',
  ...
  // Update quantity and prices when product changes
  useEffect(() => {
  ...
  const settings = {
    ...
    responsive: [
      ...
      { breakpoint: 1000, settings: { slidesToShow: 4, slidesToScroll: 2 } },
      { breakpoint: 800, settings: { slidesToShow: 3, slidesToScroll: 1 } },
      { breakpoint: 600, settings: { slidesToShow: 2, slidesToScroll: 1, nextArrow: null, prevArrow: null } },
      { breakpoint: 420, settings: { slidesToShow: 1, slidesToScroll: 1, nextArrow: null, prevArrow: null } },
    ],
  };
          ...
          <div className="show-img-detail-sup" key={uuidv4()}>
            <Suspense fallback={<Loader />}>
              <ProductCard
                ...
                moq={pro.minOrderQuant}
                id={pro.productId}
                img={pro.images && pro.images.length > 0 ? pro.images[0].imageUrl : defaulImg}
                unitPrice={pro.unitPrice}
                currencyName={pro.currencyname}
      ...
      {isClickedCart && product && (
        <div className="add-to-cart-popup add-to-cart">
          <div className="add-to-cart-wrapper-footer">
            ...
            <div className="descrip2" style={{ display: 'none' }}>
              Minimum Order Quantity: {product.minOrderQuant}
            </div>
            <div className="heading2">
              Total Price: {currencySymbols[selectedCurrency]} {convertPrice(totalPrice, product.currencyname)} {selectedCurrency}
            </div>
            <div className="plus-minus">
              <div style={{ cursor: 'pointer' }}>
                <RemoveCircleOutlineIcon onClick={decrementValue} />
              </div>
              <input className="pminput" type="number" value={value} onChange={handleInputChange} />
              <div style={{ cursor: 'pointer' }}>
                <AddCircleOutlineIcon onClick={incrementValue} />
              </div>
            </div>
            ...
            <button type="button" onClick={closepopup} className="go-home-cart">
              CLOSE
            </button>
          </div>
        </div>
